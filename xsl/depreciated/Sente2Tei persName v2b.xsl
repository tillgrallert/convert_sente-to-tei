...
<xsl:stylesheet  
    ...
    - mRep1: reproduces a TEI file
    - mRep2: links the persNames in a TEI file to a master Nym file and otherwise reproduces the TEI file
    ...
    <xsl:param name="pgModes" select="'mRep1'"/>
    ...
    <xsl:variable name="vPerson2">
        ...
        <xsl:element name="tei:listPerson">
            ...
            <xsl:for-each-group select="$vPersons2/tei:persName" group-by=".">
                ...
                <xsl:variable name="vPersNameParts" select="tokenize(normalize-space($vPersName),' ','i')"/>
