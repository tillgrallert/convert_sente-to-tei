    ...
    <xsl:variable name="vPerson2">
        ...
        <xsl:variable name="vPersNamesUnique">
            <xsl:for-each-group select="$vPersons2/tei:persName" group-by=".">
                <xsl:if test="current-grouping-key()[not(.=$vPerson//tei:persName[@type='simple'])][not(.=$pgPers//tei:listPerson[@type='mark-up']/tei:person/tei:persName[@type='simple'])]">
                    <xsl:element name="tei:persName">
                        <xsl:value-of select="."/>
                    </xsl:element>
                </xsl:if>
        ...
        <xsl:element name="tei:listPerson">
            ...
            <xsl:for-each select="$vPersNamesUnique/tei:persName">
                ...
                <xsl:if test=".[not(.=$vPerson//tei:persName[@type='simple'])][not(.=$pgPers//tei:listPerson[@type='mark-up']/tei:person/tei:persName[@type='simple'])]">
                    <xsl:element name="tei:person">
                        <xsl:attribute name="xml:id">
                            <!-- changed in v3a -->
                            <xsl:value-of select="format-number($vNCountBase+$vNCount,'p000000')"/>
                        </xsl:attribute>
                        <xsl:element name="tei:persName">
                            <xsl:element name="tei:surname">
                                <xsl:choose>
