...
<xsl:template name="templImprint">
    <xsl:element name="imprint">
        ...
        <xsl:apply-templates select="ancestor-or-self::tss:reference//tss:date[@type='Publication']"/>
    ...
    <xsl:template match="tss:date[@type='Publication']">
        ...
        <xsl:variable name="vDPubM">
            <xsl:value-of select="format-number(if(.//tss:date[@type='Publication']/@month!='') then(.//tss:date[@type='Publication']/@month) else(1),'00')"/>
        </xsl:variable>
        <xsl:variable name="vDPubD">
            <xsl:value-of select="format-number(if(.//tss:date[@type='Publication']/@day!='') then(.//tss:date[@type='Publication']/@day) else(1),'00')"/>
        </xsl:variable>
            ...
            <xsl:element name="date">
                ...
                <xsl:choose>
                    <xsl:when test="string-length($vDate)=10">
                        <xsl:value-of select="format-date($vDate,'[F,*-3], [D1] [MNn,*-3] [Y0001]','en','AD',())"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="$vDate"/>
                    </xsl:otherwise>
...
<!-- Attachments -->
    <xsl:template match="tss:attachments">
        <xsl:variable name="vAttachment">
             <xsl:value-of select="ancestor::tss:reference//tss:attachments/tss:attachmentReference[position()=1]"/>
        </xsl:variable>
        <xsl:attribute name="facs">
