...
<xsl:stylesheet  
    ...
    <!-- v2
        ...
        - PROBLEM (v2a): check for existing persNames in the pgPers in m2 and m3 and omit them from the results. This procedure currently takes far (!) too long and infeasable  -->
    <!-- v2 planned: 
    ...
    <xsl:param name="pgPers" select="document('/BachUni/projekte/XML/TEI XML/master files/persNames master TEI.xml')"/>
 ...
 <xsl:template match="tss:senteContainer">
    ...
    <xsl:variable name="vPerson">
        <!-- <xsl:variable name="vNCount">
                    <xsl:for-each select="$pgPers//tei:person">
